...
const { AiOutlinePlusCircle, AiOutlineLogout, BsChevronDown, AiOutlineUser, RiAdminLine } = icons
...
const Header = () => {
    ...
    useEffect(() => {
        setTimeout(async () => {
            ...
            if (response.success) dispatch({ type: actionTypes.IS_ADMIN, isAdmin: true })
                    ...
                    {isLoggedIn && <div className='flex items-center gap-3 relative'>
                        <div className='text-sm'>
                            {currentData && Object.keys(currentData).length > 0 && <div className='flex items-center gap-2'>
                                <img src={currentData?.image || avatar} alt="avatar" className='w-10 object-cover rounded-full h-10 border-2 shadow-md border-white' />
                        ...
                        />
                        {isShowMenu && <div className='absolute min-w-200 z-50 top-full bg-white shadow-md rounded-md p-4 right-0 flex flex-col'>
                            {currentData?.role !== 'R1' && <Link

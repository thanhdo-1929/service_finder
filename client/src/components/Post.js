...
const Post = ({ images, user, title, star, description, address, id, price, area, distance, refer, foodtypes }) => {
    ...
    return (
        <div className='w-full flex border-t border-orange-600 py-4 gap-4'>
            ...
            >
                <img src={images[0] || 'https://apollobattery.com.au/wp-content/uploads/2022/08/default-product-image.png'} alt="preview" className='w-full h-[257px] object-cover' />
            ...
            <div className='w-3/5 flex-auto flex flex-col justify-between'>
                ...
                <div className='mt-2 flex items-center gap-8'>
                    ...
                    {area ? <span className='text-xs'>{area + ' m2'}</span> : ''}
                    {distance ? <span className={`${price ? 'text-sm text-blue-500 line-clamp-1' : 'font-bold text-green-600  line-clamp-1'}`}>{
                ...
                <div className='text-gray-500 w-full h-[50px] text-ellipsis mt-2 overflow-hidden line-clamp-3'>
                    {description && description?.includes('[') && !description?.includes('<p>[')

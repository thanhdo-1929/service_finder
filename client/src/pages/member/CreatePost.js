...
import { getBase64, getDistanceFromLatLonInKm, validate } from '../../ultils/fn'
...
const CreatePost = ({ setEdit, updatingPost, isAdmin }) => {
    ...
    useEffect(() => {
        setPayload(prev => ({ ...prev, category: location.state?.category || '' }))
    }, [location])
    useEffect(() => {
        if (updatingPost) {
            setPayload({
    ...
    const handleMutilImages = async (e) => {
        if (checkFiles(e.target.files) === 0) {
            ...
            setImages(images)
        } else toast.info('Có file không phải ảnh trong số các file bạn upload lên. Hãy tải lại ảnh')
    ...
    const handleSubmit = async () => {
        if (rawImages && rawImages.length > 10) {
            ...
            if (payload.category === 'TQA') validateFields.foodType = payload.foodType
            ...
            if (updatingPost && invalids === 0) {
                ...
                if (rawImages && invalids === 0) {
                    ...
                    toast.info('Hãy điền đầy đủ thông tin ( có thể bạn đang thiếu ảnh cho bài đăng này)')
                }
            }
        }
    }
            ...
            <div className='py-8 flex gap-4 w-full'>
                <div className='flex flex-col gap-4 flex-auto w-full'>
                    <div className='flex gap-4 w-full'>
                        ...
                        {location.state?.category
                            ? <div className='flex-1'>
                                <InputField
                                    preValue={'Danh mục: '}
                                    value={categories?.find(el => el.code === location.state?.category)?.value || ''}
                                    onlyRead
                                />
                            </div>
                            : <div className='relative flex-1 '>
                                ...
                                {invalidFields?.some(i => i.name === 'category') && <div className='text-red-500 text-xs italic pl-4' >{invalidFields.find(i => i.name === 'category')?.message}</div>}
                            </div>}
                        {(location.state?.category === 'TQA' || payload.category === 'TQA') &&
                            <div className='relative flex flex-1 flex-col'>
                                ...
                                {invalidFields?.some(i => i.name === 'foodType') && <small className='text-red-500 text-xs italic pl-4' >{invalidFields.find(i => i.name === 'foodType')?.message}</small>}
                            </div>
                        }
                    </div>
                    ...
                    <div className='flex flex-col gap-2 py-4'>
                        ...
                        />
                        {invalidFields?.some(i => i.name === 'desc') && <small className='text-red-500 italic pl-4' >{invalidFields.find(i => i.name === 'desc')?.message}</small>}
                    </div>
                    <div className='flex flex-col gap-2 py-4'>
                        ...
                        {images && <div className='flex flex-wrap'>
                            {typeof images === 'string' ? <div className='w-1/4'>
                                <img src={images} alt="preview" className='h-[207px] object-contain' />
                            </div> : images?.map(el => (

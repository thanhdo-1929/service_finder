...
const ManageUser = () => {
    ...
    const fetchUsers = async (params) => {
        const response = await apiGetUsers({ ...params, limit: process.env.REACT_APP_LIMIT_ADMIN })
    ...
    const handleSubmit = async () => {
        const invalids = validate({ email: payload.email, name: payload.name }, setInvalidFields)
        if (invalids === 0) {
                            ...
                            >
                                ...
                                <td className={`p-2 ${index % 2 === 0 ? '' : 'bg-gray-100'} m-auto`}>
                                    {edit?.id === item.id
                                        ...
                                        />
                                        : <span>{item?.name}</span>}
                                    {edit?.id === item.id && invalidFields?.some(i => i.name === 'name') && <div className='text-red-500 text-xs italic' >{invalidFields.find(i => i.name === 'name')?.message}</div>}
                                </td>
                                <td className={`p-2 ${index % 2 === 0 ? '' : 'bg-gray-100'} m-auto`}>
                                    {edit?.id === item.id
                                        ? <input
                                    ...
                                    {edit?.id === item.id && invalidFields?.some(i => i.name === 'email') && <div className='text-red-500 text-xs italic' >{invalidFields.find(i => i.name === 'email')?.message}</div>}
                                </td>
                                <td className={`p-2 ${index % 2 === 0 ? '' : 'bg-gray-100'} m-auto`}>
                                    {edit?.id === item.id
